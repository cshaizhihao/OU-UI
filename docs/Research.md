...
本轮目标是补齐 v0.5.0 中“配置已写入但服务未必加载”的断点：

- Xray 与 Hysteria2 均采用每节点 managed systemd service，避免 Xray 多配置合并时出现重复 outbound tag 或全局配置冲突。
- Agent 在 `apply` 阶段原子写入 active config 和 unit 文件，随后执行 `systemctl daemon-reload`。
- `reload` 阶段改为 `systemctl enable --now` + `systemctl restart`，让 runtime 明确加载 OU-UI 生成的配置。
- systemd unit 增加基础 hardening：`NoNewPrivileges`、`PrivateTmp`、`ProtectSystem`、`ProtectHome=read-only`、`CapabilityBoundingSet`。
- 回滚阶段不再静默吞掉 systemd 错误；停止、daemon-reload、restart 失败都会反映到阶段结果。
- v0.6.0 仍不自动下载 runtime 二进制，真实安装器、端口冲突检测、UDP 健康探测和 config dry-run 留到后续版本。
...
2. 为 Xray/Hysteria2 增加真实二进制安装器和版本锁定策略。
3. 增加端口占用检测、防火墙提示、UDP 健康探测和 runtime config dry-run。
