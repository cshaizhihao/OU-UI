...
func NewRouter(cfg config.ServerConfig, db *gorm.DB) *gin.Engine {
	...
	r.GET("/", func(c *gin.Context) {
		c.JSON(http.StatusOK, gin.H{"name": "OU-UI", "version": "v3.0.0", "securePath": cfg.SecurePath})
	})
	...
	api.POST("/tenants", h.requirePanelAuth(), h.createTenant)
	api.GET("/users", h.requirePanelAuth(), h.listPanelUsers)
	api.POST("/users", h.requirePanelAuth(), h.createPanelUser)
	api.GET("/api-docs", h.requirePanelAuth(), h.apiDocs)
	api.POST("/api-keys", h.requirePanelAuth(), h.createAPIKey)
