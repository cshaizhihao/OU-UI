...
> Current release track: `v2.7.0` on `main`. This marker reflects the latest pushed implementation milestone; older historical sections below may still mention the legacy `v0.6.0` runtime baseline.

OU-UI 是一个面向自托管和多机节点运维的分布式代理节点管理面板。当前推进版本 `v2.7.0` 已在 runtime deploy lifecycle、单节点流量审计、高级路由、HA 入口和自动告警基础上，新增 SaaS 级玻璃控制台、跨平台外部订阅聚合和 Clash 规则托管。
