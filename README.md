...
OU-UI 是一个面向自托管和多机节点运维的分布式代理节点管理面板。当前版本 `v0.5.0` 已在 v0.4.0 控制链路基础上，把节点下发推进为 runtime deploy lifecycle：render、install、apply、reload、health 和 rollback。
...
v0.5.0 的 Agent 会把 `installId`、`agentId` 和 `agentToken` 保存到数据目录的 `agent-state.json`，后续重启优先复用身份；如服务端拒绝旧 token，且环境中仍有注册 token，会自动尝试重新绑定。

## Runtime Deploy

`node.deploy` 现在不再只写入渲染文件。Agent 会在被控机本地执行分阶段部署：

```text
render -> install -> apply -> reload -> health
```

- `install`：检查目标 runtime 二进制是否存在，例如 `xray`、`hysteria`。
- `apply`：写入 active config，并在已有配置时生成备份。
- `reload`：优先使用 `systemctl reload`，失败后尝试 `systemctl restart`。
...
当前 v0.5.0 不会自动安装 Xray/Hysteria2，也不会自动接管 runtime 的 systemd unit。被控机需要先准备好 `xray` 或 `hysteria-server` 服务，并确保服务配置已经加载 OU-UI 写入的数据目录；后续版本会补齐 runtime 安装器和 systemd unit 生成器。
