...
> Current release track: `v1.4.0` on `main`. This marker reflects the latest pushed implementation milestone; older historical sections below may still mention the legacy `v0.6.0` runtime baseline.

OU-UI 是一个面向自托管和多机节点运维的分布式代理节点管理面板。当前推进版本 `v1.4.0` 已在 runtime deploy lifecycle、单节点流量审计和高级路由下发基础上，新增 HA 入口解析、成员健康上报和动态后端选择闭环。
