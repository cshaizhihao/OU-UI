...
> Current release track: `v4.0.0` on `main`. This marker reflects the latest implementation milestone; older historical sections below may still mention previous runtime baselines.

OU-UI 是一个面向自托管和多机节点运维的分布式代理节点管理面板。当前推进版本 `v4.0.0` 已在 runtime deploy lifecycle、单节点流量审计、高级路由、HA 入口、自动告警、订阅聚合、Clash 托管、多租户 RBAC 和开放 API 基础上，重构为结构化 SaaS 控制台工作区，并加入默认中文、多语言切换与明暗模式。
